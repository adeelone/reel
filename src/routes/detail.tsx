  ...
  return (
    <article className="detail-page">
      <section
        ...
        style={{
          backgroundImage: `linear-gradient(90deg, rgba(9,9,11,.96), rgba(9,9,11,.42)), url(${imageUrl(item.backdropPath, 'w1280')})`,
        }}
      >
        <img className="detail-poster" src={imageUrl(item.posterPath, 'w500')} alt="" />
        <div>
          <span className="eyebrow">
            {item.year} · {item.runtime ? `${item.runtime} min` : 'Series'} · {item.certification ?? 'NR'}
          </span>
          <h1>{item.title}</h1>
          ...
          <div className="action-bar">
            <button type="button" onClick={() => toggleWatchlist(item)}>
              <ListPlus size={18} /> Watchlist
            </button>
            <button type="button" onClick={() => toggleWatched(item)}>
              <TicketCheck size={18} /> Watched
            </button>
            <button type="button" onClick={() => toggleFavorite(item)}>
              <Heart size={18} /> Favorite
            </button>
            <button type="button" onClick={() => navigator.share?.({ title: item.title, url: location.href })}>
      ...
      <section className="detail-grid">
        <div>
          ...
          <div className="credit-row">
            {[...item.cast, ...item.crew].map((credit) => (
              <span key={`${credit.id}-${credit.role}`}>
                {credit.name}
          ...
          <div className="trailer-box">
            Trailers open with youtube-nocookie embeds when live TMDB videos are configured.
          </div>
          {item.seasons.length ? <h2>Seasons</h2> : null}
          <div className="season-grid">
            {item.seasons.map((season) => (
              <a key={season.seasonNumber} href={`/tv/${item.id}/season/${season.seasonNumber}`}>
                {season.name}
                <small>{season.episodeCount} episodes</small>
              </a>
            ))}
          </div>
        </div>
        <aside>
          ...
          <label className="note-box">
            ...
            <textarea
              value={privateNote}
              onChange={(event) => note(item.id, event.target.value)}
