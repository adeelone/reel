  ...
  return (
    <div className="page">
      <div className="page-header">
        <h1>Tonight</h1>
        <button type="button" onClick={() => setSelected([...selected].reverse())}><RotateCcw size={18} /> Spin again</button>
      </div>
      <div className="chip-row">
        {moods.map((mood) => (
          <button key={mood} type="button" className={`chip ${selected.includes(mood) ? 'active' : ''}`} onClick={() => setSelected((current) => current.includes(mood) ? current.filter((entry) => entry !== mood) : [...current, mood])}>{mood}</button>
        ))}
      </div>
      <div className="poster-grid">
        {picks.map((item) => (
          <div key={item.id}>
            ...
            <button type="button" className="lock-button" onClick={() => setLocked((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id])}><Lock size={16} /> Lock</button>
          </div>
        ))}
      </div>
    </div>
